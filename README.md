...
- **Wrong destination errors (e.g., iOS Simulator not found):** Use `tools/build-for-destination.sh 'platform=iOS Simulator,name=iPhone 17'` (or another installed simulator) to get root-cause diagnostics plus available destination hints. `tools/build-simulator.sh` defaults to Vision Pro, and can target iOS with `VPSTUDIO_SIM_PLATFORM='iOS Simulator'`.
