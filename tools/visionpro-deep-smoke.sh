...
xcrun simctl launch "$DEVICE_NAME" "$BUNDLE_ID" >/dev/null 2>&1 || true
xcrun simctl terminate "$DEVICE_NAME" "$BUNDLE_ID" >/dev/null 2>&1 || true
APP_DATA="$(xcrun simctl get_app_container "$DEVICE_NAME" "$BUNDLE_ID" data)"
...
APP_BUNDLE_PATH="$(find "$HOME/Library/Developer/Xcode/DerivedData" \
  \( -path "*/Build/Products/Debug-xrsimulator/VPStudio.app" -o -path "*/Build/Products/Debug-iphonesimulator/VPStudio.app" \) \
  | sort | tail -1 || true)"
if [ -n "${APP_BUNDLE_PATH:-}" ] && [ -d "$APP_BUNDLE_PATH" ]; then
  xcrun simctl install "$DEVICE_NAME" "$APP_BUNDLE_PATH" >/dev/null 2>&1 || true
else
  echo "installStep: skipped (no built VPStudio.app found under DerivedData)" >> "$SUMMARY_TXT"
fi
...
if [ -f "$DB_PATH" ]; then
  ...
  for ((i=0; i<cycles; i++)); do
    ...
    if ! xcrun simctl launch --terminate-running-process "$DEVICE_NAME" "$BUNDLE_ID" >/dev/null 2>&1; then
...
if [ -f "$DB_PATH" ]; then
  ...
  for tab in "${tabs[@]}"; do
    sqlite3 "$DB_PATH" "INSERT OR REPLACE INTO app_settings(key,value) VALUES('last_selected_tab','${tab}');" || true
    xcrun simctl launch --terminate-running-process "$DEVICE_NAME" "$BUNDLE_ID" >/dev/null 2>&1 || true
    sleep 2.5
    tab_lc="$(printf '%s' "$tab" | tr '[:upper:]' '[:lower:]')"
    xcrun simctl io "$DEVICE_NAME" screenshot "$SHOT_DIR/vpstudio-vision-${tab_lc}.png" >/dev/null 2>&1 || true
  done
